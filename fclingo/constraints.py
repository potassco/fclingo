...
from .util import IntervalSet, TodoList, abstractproperty
        ...
        if co > 0:
            ...
            if config.refine_reasons and slack + co < 0 and ass.decision_level > 0:
                ...
                if value < current:
                    ...
                    if (
                        config.refine_introduce
                        and ass.level(lit) == ass.decision_level
                        and value < current
                    ):
    ...
    def propagate(self, state, cc, config, check_state):
        ...
        if slack < 0:
            ...
            for co, var in self.elements:
                ...
                ret, slack, lit = self._calculate_reason(
                    state, cc, slack, vs, co, config
                )
                if not ret:
                    return False
        ...
        for co_r, var_r in self.elements:
            ...
            if lit_r == 0 or not ass.is_true(lit_r):
                ...
                for co_a, var_a in self.elements:
                    ...
                    # calculate reason literal
                    ret, slack_r, lit_a = self._calculate_reason(
                        state, cc, slack_r, vs_a, co_a, config
                    )
                    if not ret:
                        return False
                ...
                if co_r > 0:
                    ret, lit_r = state.update_literal(
                        vs_r, value_r - 1, cc, not guess or None
                    )
                    if not ret:
                        return False
                else:
    ...
    def _clause_translate(self, cc, state, elements, lower, upper, config):
        ...
        while todo:
            ...
            if i > 0:
                ...
                lit = (
                    -TRUE_LIT
                    if cc.assignment.is_false(-self.literal)
                    else -self.literal
                )
...
class DistinctState(AbstractConstraintState):
    ...
    def translate(self, cc, state, config, added):
        ...
        elem_vars = []
        for (fixed, elements), (lower, upper), values in zip(
            self.constraint.elements, self.assigned.values(), elem_values
        ):
            var = None
        ...
        for co, var in self.constraint.elements[j][1]:
            ...
            if s * co > 0:
                ...
                if not ass.is_fixed(reason[-1]):
                    is_fact = False
                # add consequence
                ret, lit = state.update_literal(
                    vs, vs.upper_bound - 1, cc, is_fact or None
                )
                if not ret:
                    return False
                reason.append(lit)
                ...
                # add consequence
                ret, lit = state.update_literal(
                    vs, vs.lower_bound, cc, not is_fact and None
                )
                if not ret:
                    return False
                reason.append(-lit)
