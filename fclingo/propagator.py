...
from .util import IntervalSet, measure_time_decorator
    ...
    def add_distinct(self, literal, elems):
        ...
        for i, (rhs_i, elems_i) in enumerate(elems):
            for rhs_j, elems_j in elems[i + 1 :]:
                ...
                self.add_constraint(
                    b, [(-co, var) for co, var in celems], -rhs - 1, False
                )
...
class Propagator(object):
    ...
    def __init__(self):
        ...
        self._translated_minimize = (
            False  # whether a minimize constraint has been translated
        )
    ...
    def check(self, control):
        ...
        if not state.check(
            ControlClauseCreator(control, state.statistics), self.config.check_state
        ):
            return
    ...
    def shown(self, var):
        ...
        if (
            var.type == clingo.SymbolType.Function
            and (var.name, len(var.arguments)) in self._show_signature
        ):
            return True
    ...
    def get_assignment(self, thread_id, show_all=False):
        ...
        return self._state(thread_id).get_assignment(
            (var, idx)
            for var, idx in self._var_map.items()
            if show_all or self.shown(var)
        )
