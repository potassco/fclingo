...
from fclingo.astutil import Transformer, collect_variables, match, unpool_theory_atom
from fclingo.util import ABC, abstractproperty, gcd
...
def _parse_dom(builder, atom):
    ...
    builder.add_dom(
        builder.cc.solver_literal(atom.literal), builder.add_variable(var), elements
    )
...
def _parse_distinct(builder, atom):
    ...
    for elem in atom.elements:
        if elem.terms and not elem.condition:
            elements.append(
                simplify(_parse_constraint_elem(builder, elem.terms[0], True), False)
            )
        else:
            raise RuntimeError("Invalid Syntax")
...
def _parse_constraint(builder, atom, is_sum, strict):
    ...
    rhs, elements = simplify(
        _parse_constraint_elems(builder, atom.elements, atom.guard[1], is_sum), True
    )
...
class HeadBodyTransformer(Transformer):
    ...
    def visit_Rule(self, rule):
        ...
        if (
            self._shift
            and head.type == ast.ASTType.Literal
            and head.atom.type == ast.ASTType.BooleanConstant
            and not head.atom.value
        ):
            for literal in body:
                if (
                    literal.type == ast.ASTType.Literal
                    and literal.atom.type == ast.ASTType.TheoryAtom
                ):
                    atom = literal.atom
                    term = atom.term
                    if (
                        term.name in ["sum", "diff", "in", "max", "min", "count"]
