...
class FclingoApp(clingo.Application):
    ...
    def register_options(self, options):
        ...
        options.add_flag(
            group,
            "shift-constraints",
            "Shift constraints into head of integrity constraints [{}]".format(
                self._flag_str(self.config.shift_constraints)
            ),
        ...
        options.add_flag(
            group,
            "sort-constraints",
            "Sort constraint elements [{}]".format(
                self._flag_str(conf.sort_constraints)
            ),
        ...
        options.add(
            group,
            "translate-clauses",
            "Restrict translation to <n> clauses per constraint [{}]".format(
                conf.clause_limit
            ),
            self._parse_int(conf, "clause_limit", min_value=0),
            argument="<n>",
        )
        options.add_flag(
            group,
            "literals-only",
            "Only create literals during translation but no clauses [{}]".format(
                self._flag_str(conf.literals_only)
            ),
            conf.literals_only,
        )
        options.add(
            group,
            "translate-pb",
            "Restrict translation to <n> literals per pb constraint [{}]".format(
                conf.weight_constraint_limit
            ),
            self._parse_int(conf, "weight_constraint_limit", min_value=0),
            argument="<n>",
        )
        options.add(
            group,
            "translate-distinct",
            "Restrict translation of distinct constraint to <n> pb constraints [{}]".format(
                conf.distinct_limit
            ),
            self._parse_int(conf, "distinct_limit", min_value=0),
            argument="<n>",
        )
        options.add_flag(
            group,
            ...
            "Translate minimize constraint into clasp's minimize constraint [{}]\n".format(
                self._flag_str(conf.translate_minimize)
            ),
        ...
        options.add(
            ...
            """.format(
                    self._flag_str(conf.default_state_config.refine_reasons)
                )
            ),
            self._parse_bool_thread("refine_reasons"),
            True,
        )
        options.add(
            group,
            ...
            Introduce order literals when generating reasons [{}]
                  ...
                  <i>: Only enable for thread <i>\
            """.format(
                    self._flag_str(conf.default_state_config.refine_introduce)
                )
            ),
            self._parse_bool_thread("refine_introduce"),
            True,
        )
        options.add(
            group,
            ...
            Use closest order literal as reason [{}]
                  ...
                  <i>: Only enable for thread <i>\
            """.format(
                    self._flag_str(conf.default_state_config.propagate_chain)
                )
            ),
            self._parse_bool_thread("propagate_chain"),
            True,
        )
