...
def match(term, name, arity):
    ...
    """
    return (
        term.type in (clingo.TheoryTermType.Function, clingo.TheoryTermType.Symbol)
        and term.name == name
...
class _TermUnpooler(Visitor):
    ...
    def visit_UnaryOperation(self, x):
        ...
        """
        return [
            ast.UnaryOperation(x.location, x.operator, y) for y in self.visit(x.term)
        ]
    ...
    def visit_BinaryOperation(self, x):
        ...
        """
        return [
            ast.BinaryOperation(x.location, x.operator, l, r)
            for l, r in product(self.visit(x.left), self.visit(x.right))
        ]
    ...
    def visit_Function(self, x):
        ...
        """
        return [
            ast.Function(x.location, x.name, args, x.external)
...
class _AtomUnpooler(Visitor):
    ...
    def visit_Comparison(self, x):
        ...
        """
        return [
            ast.Comparison(x.comparison, l, r)
