...
from .util import ABC, SortedDict, TodoList, lerp, measure_time_decorator, remove_if
...
class State(object):
    ...
    def copy_state(self, master):
        ...
        for var, css in master._v2cs.items():
            self._v2cs[var] = [
                (co, self.constraint_state(cs.constraint)) for co, cs in css
            ]
    ...
    def _update_domain(self, cc, lit):
        ...
        if -lit in self._litmap:
            ...
            for vs, value in self._litmap[-lit][start:]:
                ...
                if not self._propagate_variables(
                    cc, vs, lit, vs.prev_values(value), -1
                ):
                    return False
    ...
    def add_simple(self, cc, clit, co, var, rhs, strict):
        ...
        if (
            strict
            and vs.min_bound <= value < vs.max_bound
            and not vs.has_literal(value)
        ):
            lit = clit
            if co < 0:
                lit = -lit
    ...
    def cleanup_literals(self, cc):
        ...
        # cleanup
        return self._cleanup_literals(
            cc, TRUE_LIT, lambda x: x[1] != x[0].upper_bound
        ) and self._cleanup_literals(
            cc, -TRUE_LIT, lambda x: x[1] != x[0].lower_bound - 1
        )
