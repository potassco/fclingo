...
class TestMain(unittest.TestCase):
    ...
    def test_singleton(self):
        ...
        self.assertEqual(
            solve(" :- &sum { x } <= 1. :- not &sum { x } > 1.", 0, 2), [[("x", 2)]]
        )

    def test_distinct(self):
        self.assertEqual(
            solve("&distinct { x; y }.", 0, 1),
            [[("x", 0), ("y", 1)], [("x", 1), ("y", 0)]],
        )
        self.assertEqual(
            solve("&distinct { 2*x; 3*y }.", 2, 3),
        ...
        self.assertEqual(
            solve("&distinct { y-x; x-y }.", 0, 1),
            [[("x", 0), ("y", 1)], [("x", 1), ("y", 0)]],
        )
        self.assertEqual(
            solve("&distinct { x; y } :- c. &sum { x } = y :- not c. {c}.", 0, 1),
            [
                [("x", 0), ("y", 0)],
                [("x", 1), ("y", 1)],
                ["c", ("x", 0), ("y", 1)],
        ...
        self.assertEqual(
            solve("&dom{1..1}=x. &dom{1..2}=y. &dom{1..3}=z. &distinct{x;y;z}."),
            [[("x", 1), ("y", 2), ("z", 3)]],
        )
        self.assertEqual(
            solve("&dom{1..3}=x. &dom{2..3}=y. &dom{3..3}=z. &distinct{x;y;z}."),
            [[("x", 1), ("y", 2), ("z", 3)]],
        )
        self.assertEqual(
            solve(
    ...
    def test_dom(self):
        ...
        self.assertEqual(
            solve(
                "1 {a; b} 1. &dom { 0;2;4 } = x :- a. &dom { 1;3;5 } = x :- b.", -10, 10
            ),
            [
                ["a", ("x", 0)],
                ["a", ("x", 2)],
